...
def process_extracted_text(text):
    """Process extracted text to structure it in JSON format using the LLM."""
    try:
        ...
        logger.info(f"Sending prompt to OpenRouter ({settings.OPENROUTER_MODEL})")
        response_text = generate_content(prompt)
        logger.info(f"Received response from OpenRouter ({len(response_text)} chars)")
        ...
        # Strategy 4: Extract JSON by braces
        if not structured_data:
            try:
        ...
        # Additional Trade ID extraction if not found
        ...
        meaningful_data = any(
            structured_data.get(f) != "Not specified"
            for f in ["trade_id", "trade_date", "reference_spot_price", "notional_amount", "strike_price"]
        )
...
def validate_term_sheet(structured_data):
    """Validate a structured term sheet using the RAG service."""
    try:
